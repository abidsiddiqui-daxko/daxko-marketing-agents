...
| Typography | `brand-guidelines.md` — **Barlow, with Arial as fallback** | **Three files bear on this, not two.** `typography-system.md` agrees — Barlow only, and it names Sohne as prohibited. `slide-layouts.md` specifies Sohne throughout: legacy deck spec, do not enforce. `logo-guidelines.md` Don't 8 still permits Sohne **for the text portion of a product-lockup HTML pattern only** — that narrow exception is live, so do not flag it. Flag the conflict and escalate to Clint Malson |
